...
//        Crashlytics.setString("manufacturer",Build.MANUFACTURER);
        ...
        Dexter.checkPermissions(permissionsListener, Manifest.permission.READ_PHONE_STATE, Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.BLUETOOTH_ADMIN, Manifest.permission.CAMERA);
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        try {
                            ...
                            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                                ...
                                hasInstallPermission = Settings.Secure.getInt(getContentResolver(), Settings.Secure.INSTALL_NON_MARKET_APPS) == 1;
                            }
        ...
        new android.app.AlertDialog.Builder(this).setTitle(R.string.alert_permission_setting)
                ...
                .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                    @Override public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                            startActivityForResult(new Intent(Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES, packageURI), ALLOW_UNKNOWN_TASK);
                        }else {
    ...
    private void downloadApk(final String url, final int version) {
        ...
        Log.i("install", "downloadApk: "+file.getPath());
        if (file.exists()) {
            startInstaller(file);
            return;
...
//                        progressDialog.setMessage(soFarBytes+"/"+totalBytes);
